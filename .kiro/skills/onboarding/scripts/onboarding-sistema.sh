#!/usr/bin/env bash
# ==========================================================================
# onboarding-sistema.sh — Cria e inicializa um sistema novo a partir do template
# ==========================================================================
# Uso:
#   bash onboarding-sistema.sh
#
# Pre-requisitos (verificados automaticamente):
#   - setup-ambiente.sh ja executado (Node 20+, Docker, gh autenticado)
...
# --- Funcao para converter kebab-case em PascalCase ---
to_pascal() {
...
echo -e "${CYAN}"
echo "==========================================="
echo "  Onboarding de Sistema — DETRAN-SP"
echo "==========================================="
echo -e "${NC}"
...
command -v node &>/dev/null || fail "Node.js nao instalado. Execute setup-ambiente.sh primeiro."
command -v docker &>/dev/null || fail "Docker nao instalado. Execute setup-ambiente.sh primeiro."
command -v gh &>/dev/null || fail "gh CLI nao instalado. Execute setup-ambiente.sh primeiro."
gh auth status &>/dev/null || fail "gh nao autenticado. Execute setup-ambiente.sh primeiro."
...
# Dom\u00ednio \u2014 $(to_pascal "$SISTEMA_NOME")
...
if [[ -d "${SRC_DIR}/exemplo" ]]; then
  ...
  # Substituir conteudo nos arquivos do modulo
  find "${SRC_DIR}/${MODULO_NOME}" -type f -name "*.ts" | while read -r file; do
    sed -i "s/ExemploModule/${PASCAL}Module/g" "$file"
    sed -i "s/ExemploController/${PASCAL}Controller/g" "$file"
    sed -i "s/ExemploService/${PASCAL}Service/g" "$file"
    sed -i "s/CriarExemploDto/Criar${PASCAL}Dto/g" "$file"
    sed -i "s/exemploService/${CAMEL}Service/g" "$file"
    sed -i "s/ItemExemplo/Item${PASCAL}/g" "$file"
    sed -i "s/@Controller(\"exemplo\")/@Controller(\"${MODULO_NOME}\")/g" "$file"
    sed -i "s/criar-exemplo.dto/criar-${MODULO_NOME}.dto/g" "$file"
    sed -i "s/\.\/exemplo\.service/.\/${MODULO_NOME}.service/g" "$file"
    sed -i "s/\.\/exemplo\.controller/.\/${MODULO_NOME}.controller/g" "$file"
    sed -i "s/\.\/exemplo\.module/.\/${MODULO_NOME}.module/g" "$file"
  done

  # Atualizar app.module.ts
  APP_MODULE="${SRC_DIR}/app.module.ts"
  if [[ -f "$APP_MODULE" ]]; then
    sed -i "s/ExemploModule/${PASCAL}Module/g" "$APP_MODULE"
    sed -i "s|./exemplo/exemplo.module|./${MODULO_NOME}/${MODULO_NOME}.module|g" "$APP_MODULE"
  fi
...
if [[ -f "$WORKFLOW_FILE" ]]; then
  sed -i "s/SISTEMA/${SISTEMA_NOME}/g" "$WORKFLOW_FILE"
